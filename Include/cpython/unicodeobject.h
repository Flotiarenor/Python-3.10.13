...
/* Similar to PyUnicode_FromString(), but hex points to null-terminated
   hexadecimal encoded bytes. The size is determined with strlen(). */
PyAPI_FUNC(PyObject*) PyUnicode_FromHex(
    const char* hex              /* Hexadecimal encoded string */
);
